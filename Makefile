...
lint: ## Run lint checks (markdown, yaml, pre-commit)
	...
	@pre-commit run --all-files 2>/dev/null || echo "⚠️  pre-commit not installed (pip install pre-commit && pre-commit install)"
...
test: ## Run tests (if any)
	@if [ -f pyproject.toml ]; then python -m pytest tests/ -v 2>/dev/null || echo "no pytest or no tests"; fi
...
# Git Flow helpers
# ---------------------------------------------------------------------------
# These commands require git-flow (https://github.com/nvie/gitflow)
...
git-flow-init: ## Initialize Git Flow in this repository
	@if ! git flow init -d 2>/dev/null; then \
		echo "⚠️  git-flow not installed. Using defaults:"; \
...
git-feature-start: ## Start a new feature: make git-feature-start NAME=my-feature
	...
	@git flow feature start $(NAME) || (echo "⚠️  git-flow not found. Run instead:"; echo "  git checkout develop && git pull && git checkout -b feature/$(NAME)")
...
git-feature-finish: ## Finish current feature: make git-feature-finish NAME=my-feature
	@if [ -z "$(NAME)" ]; then \
		...
		echo "⚠️  Consider creating a PR instead of finishing locally."; \
		exit 1; \
	fi
	@echo "ℹ️  To finish feature/$(NAME):"
	@echo "  1. Create a PR: feature/$(NAME) → develop"
	@echo "  2. After merge, delete the local branch:"
	@echo "     git branch -d feature/$(NAME)"
	@echo ""
	@echo "Or use git-flow: git flow feature finish $(NAME)"
...
git-release-finish: ## Finish current release: make git-release-finish VERSION=1.0.0
	@if [ -z "$(VERSION)" ]; then \
		...
		echo "⚠️  Consider creating PRs instead of finishing locally."; \
		exit 1; \
	fi
	@echo "ℹ️  To finish release/$(VERSION):"
	...
	@echo ""
	@echo "Or use git-flow: git flow release finish $(VERSION)"
...
git-hotfix-finish: ## Finish hotfix: make git-hotfix-finish VERSION=1.0.1
	...
	@echo "ℹ️  To finish hotfix/$(VERSION):"
	...
	@echo ""
	@echo "Or use git-flow: git flow hotfix finish $(VERSION)"
