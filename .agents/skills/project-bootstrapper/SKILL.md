...
## ⚠️ 絶対ルール：質問をスキップしないこと

> **CRITICAL:** This bootstrapper MUST ask the user ALL questions defined in
> `questions.yaml` BEFORE generating any project code. NEVER auto-generate
> a project without user input. The questionnaire is the CORE of this skill.
>
> **警告：無断でプロジェクトを生成しないこと。**
>
> 1. **質問なしでの実装は禁止。** たとえユーザーが「ToDoアプリを作って」と
>    言ったとしても、まず `questions.yaml` の全質問を1つずつ clarify で尋ねること。
>    過去の会話履歴や類似プロジェクトの知識で補完して勝手に進まないこと。
> 2. **全質問が終わるまで実装タスクに進まない。** Phase 0 → Phase 1-4 の
>    全質問が完了するまで、`create_repo` や `scaffold_code` 等の実装タスクは
>    一切実行してはならない。
> 3. **「デフォルトで進める」も確認必須。** デフォルト値を採用する場合も、
>    「デフォルトのXXで進めますか？」とユーザーに確認すること。
>    無断でデフォルト適用して先に進まない。
> 4. **実装タスクリスト生成後も確認必須。** タスクリストを生成したら、
>    ユーザーに一覧提示して実行承認を得てからタスク実行に進むこと。
...
```
# エージェントに以下のように指示（質問が先、実装は後）：
# 「このテンプレートを使って新しいプロジェクトをブートストラップして」
#（上記の絶対ルールに従い、まず全質問を clarif で行うこと）
```
...
ユーザーがタスクリストを確認して承認したら、生成されたタスクリストを順番に実行する。

> **⚠️ 実行開始前に確認：**
> - Phase 0〜4の全質問は終わったか？
> - 回答は全て questions.yaml に記録したか？
> - タスクリストをユーザーに提示して承認を得たか？
> 上記のいずれかが NO なら、まだ実行に進んではならない。質問に戻れ。
