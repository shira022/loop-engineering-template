...
```
/home/shira/project/<name>/
├── repo-<name>          # メインリポジトリ（クローン先）
└── worktrees/           # git worktree 格納ディレクトリ
```

```bash
git clone <clone-url> /home/shira/project/<name>/repo-<name>
mkdir -p /home/shira/project/<name>/worktrees/
```
...
`/home/shira/project/repo-registry.yaml` に以下のフォーマットでプロジェクト情報を追記します：

```yaml
- name: <project-name>
  path: /home/shira/project/<name>/repo-<name>
...
```bash
rm /home/shira/project/<name>/repo-<name>/.agents/skills/project-bootstrapper/SKILL.md
```
...
✅ プロジェクト <name> のブートストラップが完了しました。
   リポジトリ: <clone-url>
   ローカル:  /home/shira/project/<name>/repo-<name>
   言語:      <language>
   可視性:    <public|private>
   説明:      <description>
...
- プロジェクト名が既に `/home/shira/project/repo-registry.yaml` に存在する場合は上書き確認を行います。
